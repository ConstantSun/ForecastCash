...
for (k in atm_ids) 
{
  ...
  test_tbl <- atmkclean %>%  filter((datadate >= max(datadate) - 48) & (datadate < max(datadate)-7))
  
  train_tbl <- train_tbl 
  test_tbl <- test_tbl 
  
  forecast_tbl <- atmkclean %>%  filter((datadate >= max(datadate) - 7) & (datadate <= max(datadate)))
  ...
  #Investigate error
  error_tbl <- atmkclean %>% 
    filter(datadate >= max(datadate) - 7 & datadate <= max(datadate)) %>% 
